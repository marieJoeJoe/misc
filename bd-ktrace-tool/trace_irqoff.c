...
static u64 div64_ret = 0;
static u32 div64_mod = 0;

#define U64DIVISION(R,X,Y,MOD)	\
({								\
	u64 x=X;					\
	u64 y=Y;					\
	MOD = do_div(x,y);			\
...
#else
//#define MAX_TRACE_ENTRIES	U64DIVISION(div64_ret,SZ_1K , sizeof(unsigned long),div64_mod)
#define MAX_TRACE_ENTRIES		256
#define MAX_STACE_TRACE_ENTRIES	16
#endif
...
#if  !defined(__arm__)
	mod_timer_pinned(timer, jiffies + msecs_to_jiffies(sampling_period / 1000000UL));
#else
	mod_timer_pinned(timer, jiffies + msecs_to_jiffies(U64DVD(sampling_period , 1000000UL)));
#endif
#else
#if  !defined(__arm__)
	mod_timer(timer, jiffies + msecs_to_jiffies(sampling_period / 1000000UL));
#else
	mod_timer(timer, jiffies + msecs_to_jiffies(U64DVD(sampling_period , 1000000UL)));
#endif
#endif
}
...
#else
    timer->expires = jiffies + msecs_to_jiffies(U64DVD(sampling_period , 1000000UL));
#endif
...
#else
		scale_min = unlikely(i == 0) ? 1 : U64DVD(scale_max, 2);
		num = U64DVD(hist[i] * NUMBER_CHARACTER , count_max);
#endif
...
static void distribute_show_one(struct seq_file *m, void *v, bool hardirq)
{
	...
	histogram_show(m, hardirq ? "hardirq-off:" : "softirq-off:",
		       latency_count, MAX_LATENCY_RECORD,
		       U64DVD(sampling_period << 1,1000 * 1000UL));
#endif
}
...
#else
	} else if (latency < U64DVD(sampling_period << 1, 1000 * 1000UL))
#endif
		return -EINVAL;
	...
	for_each_online_cpu(cpu) {
		...
		for (i = 0; i < nr_irqoff_trace; i++) {
			...
			seq_printf(m, "%*cCOMMAND: %s PID: %d LATENCY: %llu%s\n",
				   ...
				   stack_trace->pids[i],
#if  !defined(__arm__)
				   (u64)stack_trace->latency[i].nsecs / (1000 * 1000UL),
#else
				   (u64)U64DVD(stack_trace->latency[i].nsecs , (1000 * 1000UL)),
#endif
...
#else
	seq_printf(m, "trace_irqoff_latency: %llums\n\n",
		   U64DVD(trace_irqoff_latency , (1000 * 1000UL)));
#endif
	seq_puts(m, " hardirq:\n");
...
#else
	seq_printf(m, "%llums\n", U64DVD(sampling_period , (1000 * 1000UL)));
#endif
	return 0;
}
